...
#define CHECK_ENTSIZE(section, i, type)					\
  ...
  for (i = 0, section = filedata->section_headers;
	...
	printf ("%s\n      ", printable_section_name (filedata, section));
		...
		// In case it was NULL we do not add the postfix:
		if (section->sh_type == NULL)
			print_symbol (-17, SECTION_NAME_PRINT (section));
		else {
			// Copying the section name:
			char* ATAM_Copy = (char*) malloc (sizeof(char) * strlen(SECTION_NAME_PRINT (section)));
			strcpy(ATAM_Copy, SECTION_NAME_PRINT (section));

			// Calling the print function with the postfix:
			print_symbol (-17,strcat(ATAM_Copy, "_<3LEE"));
